...
function showList(type, el) {
	...
	type === "income"
		? incomeUlList.appendChild(elList)
		: outgoUlList.appendChild(elList);
}
...
function editUlList(properUlList, properList, value, name) {
	...
	[...properUlList.querySelectorAll("li")][
		clickedIndex
	].innerHTML = `<span>${name}</span><span class= "money">${value}</span>`;
	[...properUlList.querySelectorAll("li")][clickedIndex].appendChild(editBtn);
	[...properUlList.querySelectorAll("li")][clickedIndex].appendChild(
		deleteBtn
	);
}
...
function changePosition(e, type) {
	...
	if (type === "income") {
		incomeValueArea.value != ""
			? (value = parseFloat(incomeValueArea.value))
			: value;
		name = incomeNameArea.value;
	} else {
		outgoValueArea.value != ""
			? (value = parseFloat(outgoValueArea.value))
			: value;
...
incomeAddButton.addEventListener("click", (e, type) => {
	changePosition(e, (type = "income"));
});
outgoAddButton.addEventListener("click", (e, type) => {
	changePosition(e, (type = "outgo"));
});
...
incomeUlList.addEventListener("click", (e, type = "income") =>
	clickEditBtn(e, "income")
);
outgoUlList.addEventListener("click", (e, type = "outgo") =>
	clickEditBtn(e, "outgo")
);
...
function summarizeAll() {
	...
	if (sum > 0) {
		...
		info = `Bilans jest ujemny. Jesteś na minusie  ${Math.abs(
			sum
		)} złotych.`;
	} else {
